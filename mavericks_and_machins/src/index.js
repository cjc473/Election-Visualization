...
function renderData() {
  CongressApi.getData(116)
    .then(data => {
      const senators = data.results[0].members.slice(0, 30);
      const editedSenators = senators.map(senator => {
        const name = senator.last_name + ", " + senator.first_name;
        const score = senator.missed_votes
        return {name, score}
      })
      renderGraph(editedSenators);
    })
}
